...
internals.getRootRealm = (serverOrRealm, isRealm) => {

    let realm = isRealm ? serverOrRealm : serverOrRealm.realm;
...
internals.schwifty = (realm, config) => {
    ...
    const rootRealm = internals.getRootRealm(realm, true);
    const { collector } = internals.state(rootRealm);
