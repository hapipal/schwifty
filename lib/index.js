    ...
    register: (server, options) => {
        ...
        if (!rootState.setup) {
            ...
            server.expose('commands', {
                migrate: async (initializedServer, args, rootDir) => {
                    ...
                    const realmsWithMigrationsInRoot = [];

                    realmsWithMigrationsDir.forEach((realm) => {

                        const migrationsDir = realm.plugins.schwifty.migrationsDir;
                        if (migrationsDir.includes(absRootDir)) {
                            realmsWithMigrationsInRoot.push(realm);
                        }
                    });
                    ...
                    // These models may have been extended so let's grab them from the
                    // initialized server instead of the plugin
                    const allModels = initializedServer.models();

                    const realmKnex =
                    const realmModels = migrationRealm.models().map((model) => model.name);

                    const models = allModels.filter((model) => realmModelNames.includes(model.name));
                    const modelsArr = Object.keys(models).map((key) => models[key]);

                    /////////////////
                    console.log('modelsArr', modelsArr);

                    return Util.promisify(SchwiftyMigration.genMigrationFile)({
                        models: modelsArr,
                        migrationsDir: argsMigrationsDir || realmsWithMigrationsInRoot[0],
                        knex: migrationRealm.knex(),
                        mode,
                        migrationName
                    });
                }
