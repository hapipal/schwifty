...
exports.plugin = {
    ...
    register: function (server, options) {
        ...
        const rootState = internals.state(server.realm.parent);
        ...
        const config = internals.registrationConfig(options, server.realm.parent);
...
internals.registrationConfig = (options, rootServer) => {
    ...
    if (typeof config.models === 'string') {
        if (Path.isAbsolute(config.models)) {
            ...
            const relativeTo = rootServer.settings.files.relativeTo || '';
...
internals.initialize = (server) => {
    ...
    Items.parallel(knexes, ping, (err) => {
        ...
        Migrator.migrate(collector.knexGroups, rootKnex, rollback);
    });
};
...
internals.schwifty = function (config) {
    ...
    const state = internals.state(this.realm.parent);
    const collector = internals.state(this.realm.parent).collector;
...
internals.models = (getServer, realmPath) => {
    ...
    //console.log(getServer.toString());
    ...
    return function (all) {
        ...
        const collector = internals.state(server.realm).collector;
...
internals.state = (srv) => {
    //console.log(srv);
    const state = srv.plugins.schwifty = srv.plugins.schwifty || {};
    return state;
};

internals.stop = function (server, next) {

    const collector = internals.state(server.realm.parent).collector;
