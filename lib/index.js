...
const SchwiftyMigrateDiffHelper = require('./schwifty-migrate-diff');
...
exports.plugin = {
    ...
    register: (server, options) => {
        ...
        if (!rootState.setup) {
            ...
            server.expose('commands', { 'migrate:diff': internals.migrateDiffCmd });
...
internals.migrateDiffCmd = async (srv, args, rootDir, ctx) => {

    const SchwiftyMigrateDiff = SchwiftyMigrateDiffHelper.get();
    ...
    const parsedArgs = Bossy.parse(definition, { argv: args });

    if (parsedArgs instanceof Error) {
        throw new ctx.DisplayError(parsedArgs.message + Os.EOL + Bossy.usage(definition));
    }

    Hoek.assert(parsedArgs._ && parsedArgs._.length !== 0, new ctx.DisplayError('A name must be specified for the generated migration'));
    Hoek.assert(parsedArgs._.length === 1, new ctx.DisplayError(Bossy.usage(definition)));

    const [migrationName] = parsedArgs._;

    if (parsedArgs.mode) {
        Hoek.assert(parsedArgs.mode === 'create' || parsedArgs.mode === 'alter', new ctx.DisplayError(Bossy.usage(definition)));
    }

    const { realmsWithMigrationsDir } = internals.state(srv.realm).collector;

    const realmsWithMigrationsInRoot = Array.from(realmsWithMigrationsDir).reduce((collector, realm) => {

        const { migrationsDir } = internals.state(realm);
        if (internals.isChildPath(migrationsDir, rootDir)) {
            collector.push(realm);
        }

        return collector;
    }, []);

    Hoek.assert(realmsWithMigrationsInRoot.length > 0, new ctx.DisplayError(`At least 1 migrations directory must be present inside ${rootDir}`));
    Hoek.assert(realmsWithMigrationsInRoot.length === 1, new ctx.DisplayError(`Only 1 migrations directory supported per project at this time. Please remove one of [${realmsWithMigrationsInRoot.map((realm) => realm.plugins.schwifty.migrationsDir)}]`));

    const [migrationRealm] = realmsWithMigrationsInRoot;

    const serverModels = srv.models();
    const allModels = Object.keys(serverModels).map((modelName) => serverModels[modelName]);
    const realmKnex = internals.state(migrationRealm).knex;
    const models = allModels.filter((model) => model.knex() === realmKnex);
    const modelsArr = Object.keys(models).map((key) => models[key]);

    return await Util.promisify(SchwiftyMigrateDiff.genMigrationFile)({
        models: modelsArr,
        migrationsDir: parsedArgs.dir || internals.state(migrationRealm).migrationsDir,
        knex: realmKnex,
