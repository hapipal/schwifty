    ...
    register: (server, options) => {
        ...
        if (!rootState.setup) {
            ...
            server.expose('commands', {
                'migrate:diff': (initializedServer, args, rootDir) => {

                    let SchwiftyMigrateDiff;
                    try {
                        SchwiftyMigrateDiff = require('schwifty-migrate-diff');
                    }
                    catch (err) {
                        return Promise.resolve('To use this command please first install `npm install --save-dev schwifty-migrate-diff`');
                    }

                    const [mode, migrationName, argsMigrationsDir] = args;

                    const realmsWithMigrationsDir = internals.state(initializedServer.realm).collector.realmsWithMigrationsDir;

                    const absRootDir = Path.isAbsolute(rootDir) ?
                    ...
                    const realmsWithMigrationsInRoot = Array.from(realmsWithMigrationsDir).reduce((collector, realm) => {
                        ...
                        const migrationsDir = internals.state(realm).migrationsDir;
                        if (migrationsDir.includes(absRootDir)) {
                    ...
                    Hoek.assert(realmsWithMigrationsInRoot.length > 0, `At least 1 migrations directory must be present inside ${absRootDir}`);
                    Hoek.assert(realmsWithMigrationsInRoot.length === 1, `Only 1 migrations directory supported per project at this time. Please remove one of [${realmsWithMigrationsInRoot.map((realm) => realm.plugins.schwifty.migrationsDir)}]`);

                    const [migrationRealm] = realmsWithMigrationsInRoot;

                    const serverModels = initializedServer.models();
                    ...
                    const models = Array.from(allModels).filter((model) => model.knex() === realmKnex);
                    const modelsArr = Object.keys(models).map((key) => models[key]);

                    return Util.promisify(SchwiftyMigrateDiff.genMigrationFile)({
                        models: modelsArr,
                        migrationsDir: argsMigrationsDir || internals.state(migrationRealm).migrationsDir,
                        knex: realmKnex,
                        mode,
                        migrationName
                    });
