...
exports.assertCompatible = (A, B, msg) => {
    ...
    const nameMatches = A.name === B.name;

    Hoek.assert(isExtension && nameMatches, msg || 'Models are incompatible.  One model must extend the other, and they must have the same name.');
};
...
internals.initialize = (server, next) => {
    ...
    Items.parallel(knexes, ping, (err) => {
        ...
        const migrationGroups = collector.knexGroups.map((knexGroup) => {
            ...
            if (!knexGroup.knex) {
                knexGroup.knex = rootKnex;
            }

            // Default to passed-in migrations directory

            if (!knexGroup.migrationsDir) {
                knexGroup.migrationsDir = rootMigrationsDir;
            }

            // Turn these strings into the model classes

            const allModels = server.models(true);
            knexGroup.models = knexGroup.models.map((modelName) => {
        ...
        // Run schwifty-migration to generate migration files

        const skipMigrations = process.env.MIGRATE !== 'true';

        if (skipMigrations) {
            console.log('Skipping migrations');
            // Skip SchwiftyMigration
            return next();
        }

        SchwiftyMigration.genMigrationFile(server, { migrationGroups }, (err) => {
            ...
            Migrator.migrate(migrationGroups, rollback, next);
        });
    });
};
