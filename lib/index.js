...
exports.register = function (server, options, next) {
    ...
    if (!rootState.setup) {
        ...
        server.decorate('server', 'hapiObjection', internals.hapiObjection);
    ...
    if (typeof options.teardownOnStop !== 'undefined') {
        Hoek.assert(collector.teardownOnStop === null, 'HapiObjection\'s teardownOnStop option can only be specified once.');
        collector.teardownOnStop = options.teardownOnStop;
    }

    const config = internals.registrationConfig(options);
    server.root.hapiObjection(config);
...
exports.Model = class hapiObjectionModel extends Model {
...
internals.initialize = function (server, next) {
    ...
    Object.keys(collector.models).forEach((id) => {
        ...
        collector.models[id] = collector.models[id];
    });
...
internals.hapiObjection = function (config) {

    let isModels = false;

    // Array of models, coerce to config
    if (Array.isArray(config)) {
        config = Joi.validate(config, Schema.hapiObjection);
        isModels = true;
    } else {
        config = Joi.validate(config, Schema.hapiObjection);
    }
...
internals.collections = (serverFrom, realmPath) => {
    ...
    return function (all) {
        ...
        const collections = {};
        const models = Hoek.reach(this, `${realmPath}.plugins.hapiObjection.models`) || [];

        for (let i = 0; i < models.length; ++i) {
            const modelName = models[i].toLowerCase();
            collections[modelName] = rootCollector.models[modelName];
        }
...
internals.state = (srv) => {
    ...
    const state = srv.realm.plugins.hapiObjection = srv.realm.plugins.hapiObjection || {};
