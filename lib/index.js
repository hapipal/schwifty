        ...
        if (!rootState.setup) {
            ...
            server.expose('commands', {
                migrate: async (initializedServer, args, rootDir) => {
                    ...
                    const migrationDirs = await internals.rSearchForDirs(rootDir, 'migrations');

                    Hoek.assert(migrationDirs.length > 0, `At least 1 "migrations" directory must be preset inside ${rootDir}`);
                    Hoek.assert(migrationDirs.length === 1, `Only 1 "migrations" directory supported at this time. Please remove one of ${migrationDirs}`);

                    const [mode, migrationName, argsMigrationsDir] = args;

                    const models = initializedServer.models();
                    const modelsArr = Object.keys(models).map((key) => models[key]);

                    return Util.promisify(SchwiftyMigration.genMigrationFile)({
                        models: modelsArr,
                        migrationsDir: argsMigrationsDir || migrationDirs[0],
                        knex: initializedServer.knex(),
                        mode,
                        migrationName
                    });
                }
...
internals.isDir = (path) => Fs.lstatSync(path).isDirectory();
internals.rSearchForDirs = (rootDir, searchName) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        Fs.readdir(rootDir, async (err, files) => {
            ...
            const filesAbsolutePaths = files.map((fName) => Path.join(rootDir, fName));
            const folders = filesAbsolutePaths.filter(internals.isDir);
            const matchingFolders = folders.filter((fName) => fName.toLowerCase().split('/').pop().includes(searchName.toLowerCase()));

            let newMatches = matchingFolders;

            for (let i = 0; i < folders.length; ++i) {

                const fName = folders[i];
                const result = await internals.rSearchForDirs(fName, searchName);
                newMatches = newMatches.concat(result);
            };

            return resolve(newMatches);
        });
    });
};
