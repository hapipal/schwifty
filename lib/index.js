...
internals.migrateDiffCmd = async (srv, args, rootDir, ctx) => {
    ...
    if (!migrationName || migrationName.startsWith('--')) {
        throw new ctx.DisplayError('A name must be specified for the generated migration');
    }
    ...
    const parsedArgs = ctx.Bossy.parse(definition, { argv: args });

    if (parsedArgs.mode) {
        Hoek.assert(parsedArgs.mode === 'create' || parsedArgs.mode === 'alter', new ctx.DisplayError(ctx.Bossy.usage(definition)));
    }
