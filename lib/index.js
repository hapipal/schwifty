    ...
    register: (server, options) => {
        ...
        if (!rootState.setup) {
            ...
            server.expose('commands', {
                migrate: (initializedServer, args, rootDir) => {

                    console.log('rootDir', rootDir);

                    // args order is [mode, migrationName, migrationsDir]
                    ...
                    return Util.promisify(SchwiftyMigration.genMigrationFile)({
                        models: modelsArr,
                        migrationsDir: args[2] || options.migrationsDir,
...
internals.searchForDirs = (rootDir, searchName) => {
