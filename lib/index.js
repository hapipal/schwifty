...
internals.schwifty = function (config) {
    ...
    /*
        A knexGroup's purpose is to specify which models go with which (db) connection.
    ...
    */
    ...
    if (state.knexGroupId) {
        ...
        // The knexGroupId exists so add to models already there, also no need to continue
...
internals.models = (serverFrom, realmPath) => {
    ...
    return function (all) {
        ...
        if (knexGroupId) {
            models = rootCollector.knexGroups[knexGroupId].models || [];
        }
