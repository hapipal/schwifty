...
exports.plugin = {
    ...
    register: (server, options) => {
        ...
        const rootRealm = Toys.rootRealm(server.realm, 'schwifty');
        const rootState = Toys.rootState(server.realm, 'schwifty');
        ...
        internals.schwifty(rootRealm, config);
    }
};
...
internals.initialize = async (server) => {
    ...
    const { collector } = Toys.rootState(server.realm, 'schwifty');
...
internals.schwifty = (realm, config) => {
    ...
    const state = Toys.state(realm, 'schwifty');
    const rootState = Toys.rootState(realm, 'schwifty');
...
internals.models = (getRealm) => {
    ...
    return function (all) {
        ...
        const { models } = all ? Toys.rootState(realm, 'schwifty') : Toys.state(realm, 'schwifty');
