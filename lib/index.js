...
internals.initialize = function (server, next) {
    ...
    Object.keys(collector.models).forEach((id) => {
        ...
        const modelKnexed = model.bindKnex(server.knex());
...
internals.stop = function (server, next) {
    ...
    return server.knex().destroy(next);
};

internals.schwifty = function (config) {

    let isModels = false;
    let userConfig = Hoek.shallow(config);

    if (Array.isArray(userConfig) || userConfig.prototype instanceof Objection.Model) {
    ...
    modelIds.forEach((id, index) => {
        ...
        collector.models[id] = userConfig.models[index];
    ...
    if (userConfig.knexConfig) {
        ...
        if (typeof userConfig.knexConfig !== 'undefined') {
            Hoek.assert(typeof this.knex() === 'undefined', 'Only one knexConfig allowed per server or plugin');
        }

        state.knex = Knex(userConfig.knexConfig);
    }
};
