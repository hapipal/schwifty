...
internals.migrateDiffCmd = async (srv, argv, rootDir, ctx) => {
    ...
    if (!migrateDiff) {
        throw new DisplayError(colors.yellow('To use this command please first install `npm install --save-dev schwifty-migrate-diff@next`'));
    }
    ...
    const realm = realmsWithMigrationsInRoot[0];
    const knex = internals.knex(() => realm)();
    const { migrationsDir } = internals.state(realm);
    ...
    const { code, file, skippedColumns } = await Util.promisify(migrateDiff.genMigrationFile)({
        knex,
        models,
        migrationsDir,
        migrationName,
    ...
    if (Object.keys(skippedColumns).length) {
        const schemaConflictMsg = '(This column is in one of your Joi schemas)';
        const skippedColsOutput = skippedColumns.reduce((msg, skippedCol) => {

            return msg += `"${skippedCol.column}": "${skippedCol.type}" ${skippedCol.schemaConflict ? schemaConflictMsg : ''}\n`;
        }, 'Detected unsupported db columns that were ignored (schwifty-migrate-diff may just not know about these yet):\n\n"column": "type"\n\n');
        ctx.options.err.write(colors.yellow(`${skippedColsOutput}\n`));
    }
    ...
    return { code, file, skippedColumns };
};
