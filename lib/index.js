...
exports.register = function (server, options, next) {
    ...
    internals.state(server).knex = Knex(knexFile[process.env.NODE_ENV]);
    ...
    const defaults = knexFile.defaults || {};
...
internals.initialize = function (server, next) {
    ...
    SchwiftyMigration.initModels(modelsAsArray, internals.state(server).knex, internals.state(server).dir, (err, res) => {
...
internals.models = (serverFrom, realmPath) => {
    ...
    return function (all) {
        ...
        const rootCollector = internals.state(serverFrom.root).collector;
