...
exports.migrate = (migrationGroups, defaultKnex, rollback, cb) => {

    const knexMigrations = new Map();

    // Compile migration info per knex instance

    for (let i = 0; i < migrationGroups.length; ++i) {
        const mGroup = migrationGroups[i];
        const migrationsDir = mGroup.migrationsDir; // Already normalized to abs path

        if (!migrationsDir) {
            continue;
        }

        const knex = mGroup.knex || defaultKnex;
